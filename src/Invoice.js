...
function InvoiceForm() {
  ...
  const fetchInvoiceData = () => {
    axios.get('http://localhost:9999/invoice')
      .then(response => {
        setInvoiceData(response.data); // Assuming response.data is the invoice object
        setLoading(false);
      })
      .catch(error => {
  ...
  const calculateTotalAmount = (rows) => {
    ...
    setInvoiceData(prevInvoiceData => ({
      ...prevInvoiceData,
      totalAmount: totalAmount.toFixed(2) // Adjust to your formatting needs
    }));
  };

  const handleSaveInvoice = () => {
    axios.put('http://localhost:9999/invoice', invoiceData)
      .then(response => {
        console.log('Invoice saved successfully:', response.data);
        setShowPreview(false);
        fetchInvoiceData(); // Refresh invoice data after saving
      })
      .catch(error => {
